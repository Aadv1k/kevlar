...
void kevlar_parse_stylesheet(char *line, FILE *out_file_buffer,
                             char *theme_name) {
    char stylesheet_path[CONFIG_MAX_PATH_SIZE];
    char stylesheet[TEMPLATE_MAX_FILE_SIZE] = "";

    snprintf(stylesheet_path, CONFIG_MAX_PATH_SIZE, "%s/%s/%s", "templates",
             theme_name, strchr(line, '/') + 1);
...
void kelvar_parse_header_and_footer(FILE *out_file_buffer,
    ...
    while (fgets(line, TEMPLATE_MAX_LINE_SIZE, header_file) != NULL) {
        ...
        if (strstr(line, "--TITLE--")) {
            kevlar_parse_template_token(
                line, "--TITLE--", kev_config->configTitle, out_file_buffer);
            continue;
        } else if (strstr(line, "--AUTHOR--")) {
            kevlar_parse_template_token(
                line, "--AUTHOR--", kev_config->configAuthor, out_file_buffer);
            continue;
        } else if (strstr(line, "--STYLE")) {
            kevlar_parse_stylesheet(line, out_file_buffer,
...
void kevlar_parse_template(FILE *in_file_buffer, FILE *out_file_buffer,
                           KevlarConfig *kev_config) {
    char line[TEMPLATE_MAX_LINE_SIZE];
    ...
    while ((fgets(line, TEMPLATE_MAX_LINE_SIZE, in_file_buffer)) != NULL) {
        if (strstr(line, "--TITLE--")) {
            ...
            kevlar_parse_template_token(line, "--LISTING--",
                                        kev_config->configListing,
                                        out_file_buffer);
            continue;
        } else if (strstr(line, "--STYLE")) {
            kevlar_parse_stylesheet(line, out_file_buffer,
            ...
            kevlar_parse_template_token(line, "--CONTENT--",
                                        kev_config->configHtmlContents,
                                        out_file_buffer);
            continue;
        } else {
            fprintf(out_file_buffer, "%s", line);
...
void kevlar_build_template(char *in_file_path, char *out_file_path,
                           KevlarConfig *kev_config) {
    ...
    if (infile == NULL) {
        fprintf(stderr,
                "[kevlar] couldn't find %s while building the template\n",
                in_file_path);
        exit(1);
    }
