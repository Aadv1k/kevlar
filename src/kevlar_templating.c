...
void kevlar_parse_stylesheet(char * line, FILE * out_file_buffer, char * theme_name) {
    ...
    snprintf(stylesheet_path, CONFIG_MAX_PATH_SIZE, "%s/%s/%s", "templates", theme_name, strchr(line, '/')+1);
    *(strchr(stylesheet_path, '-')) = '\0';
    ...
    while ((fgets(file_line, TEMPLATE_MAX_LINE_SIZE, css_file_buffer)) != NULL) strcat(stylesheet, file_line);
