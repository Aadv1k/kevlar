...
void kevlar_load_config(char file_path[CONFIG_MAX_PATH_SIZE],
    ...
    sprintf(kev_config->configFooterPath, "./%s/%s/%s", "templates",
            kev_config->configTheme, "footer.html");
    sprintf(kev_config->configHeaderPath, "./%s/%s/%s", "templates",
            kev_config->configTheme, "header.html");
    sprintf(kev_config->configIndexPath, "./%s/%s/%s", "templates",
            kev_config->configTheme, "index.html");
    sprintf(kev_config->configPostPath, "./%s/%s/%s", "templates",
...
void kevlar_generate_skeleton_config(char file_path[CONFIG_MAX_PATH_SIZE]) {
    ...
    if (file_buf == NULL) {
        fprintf(
            stderr,
            "[kevlar] something went wrong while generating sample config\n");
        exit(1);
    };
