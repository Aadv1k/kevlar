...
Md_Ast *kevlar_md_process_text_node(const char *source, size_t *pos, bool allow_line_break) {
	...
	for (size_t i = *pos; i <= src_len; ++i) {
		if (i + 1 > src_len || source[i] == '\n') {
            ...
            // NOTE: this means that our cursor didn't advance; return NULL
            if (text_buffer_pos == 0) 
                return ast_node;

			if ((ast_node->opt.text_opt.data = malloc(sizeof(char) * text_buffer_pos)) == NULL) 
                goto parsing_error;

			ast_node->opt.text_opt.len = text_buffer_pos;
			strncpy(ast_node->opt.text_opt.data, text_buffer, text_buffer_pos);
